...
int sif_cmd_opt_data(u32 cmd, u32 opt, const void *pkt,
...
EXPORT_SYMBOL_GPL(sif_cmd_opt_data);

int sif_cmd_data(u32 cmd, const void *pkt, size_t pktsize,
	iop_addr_t dst, const void *src, size_t nbytes)
{
	return sif_cmd_opt_data(cmd, 0, pkt, pktsize, dst, src, nbytes);
}
EXPORT_SYMBOL_GPL(sif_cmd_data);

int sif_cmd_opt(u32 cmd, u32 opt, const void *pkt, size_t pktsize)
{
	return sif_cmd_opt_data(cmd, opt, pkt, pktsize, 0, NULL, 0);
}
EXPORT_SYMBOL_GPL(sif_cmd_opt);

int sif_cmd(u32 cmd, const void *pkt, size_t pktsize)
{
	return sif_cmd_data(cmd, pkt, pktsize, 0, NULL, 0);
}
...
static void cmd_call_handler(const struct sif_cmd_header *header)
{
	const struct sif_cmd_handler *handler = handler_from_cmd(header->cmd);
	...
	const void *send, size_t send_size, size_t recv_size)
{
	...
	err = sif_cmd_data(SIF_CMD_RPC_CALL, &call, sizeof(call),
...
static void cmd_rpc_end(const struct sif_cmd_header *header, void *arg)
{
	const struct sif_rpc_request_end_packet *packet =
		sif_cmd_payload(header);
...
static void cmd_rpc_bind(const struct sif_cmd_header *header, void *arg)
{
	const struct sif_rpc_bind_packet *bind = sif_cmd_payload(header);
	const struct sif_rpc_request_end_packet packet = {
		.client    = bind->client,
...
static void cmd_irq_relay(const struct sif_cmd_header *header, void *arg)
{
	const struct {
		u32 irq;
	} *packet = sif_cmd_payload(header);
